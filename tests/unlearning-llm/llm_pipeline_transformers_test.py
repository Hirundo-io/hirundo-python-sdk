...
def test_public_model_classes_forward_base_model_path(
    ...
    forwarded_calls: list[tuple[LlmModel | LlmModelOut, str, str | Path | None]] = []
    pipeline_result = PipelineSentinel()

    def fake_pipeline_loader(
        llm_model: LlmModel | LlmModelOut,
        run_id: str,
        config: None = None,
        device: None = None,
        ...
        forwarded_calls.append((llm_model, run_id, base_model_path))
        return pipeline_result
    ...
    assert pipeline_output is pipeline_result
    assert forwarded_calls == [(model, "run-123", client_model_path)]
